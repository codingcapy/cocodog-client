...
export default function ProfilePage() {
    ...
    async function handleEditPassword(e) {
        ...
        if (res?.data.success) {
            navigate(`/cocodog-client/users/${userId}`);
        }
    }
    ...
    return (
        <div>
            ...
            {data.userPosts.length === 0 ? <p>You haven't posted anything yet!</p> : data.userPosts.map((post) => <div key={post.id} className="py-3">
                <Link to={`/cocodog-client/posts/${post.id.toString()}`} className="">
            ...
            {data.userComments.length === 0 ? <p>You haven't added any comments yet!</p> : data.userComments.map((comment) => <div key={comment.id} className="my-3">
                <Link to={`/cocodog-client/cocodog-client/posts/${comment.postId.toString()}`} className="">
                    <p><strong>{comment.username}</strong> {comment.date}</p>
                    <p>{comment.content}</p>
                </Link>
            </div>)}
            <h2 className="py-5 text-2xl text-slate-700 font-medium">Your Replies</h2>
            {data.userReplies.length === 0 ? <p>You haven't added any replies yet!</p> : data.userReplies.map((reply) => <div key={reply.id} className="my-3">
                <Link to={`/cocodog-client/cocodog-client/posts/${reply.postId.toString()}`} className="">
